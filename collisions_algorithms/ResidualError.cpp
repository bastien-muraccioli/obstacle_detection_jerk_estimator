#include "ResidualError.h"

ResidualError::ResidualError(void) {}

void ResidualError::init(mc_control::MCGlobalController & controller)
{
...
void ResidualError::computeMomemtum(mc_control::MCGlobalController & controller)
{
    auto & ctl = static_cast<mc_control::MCGlobalController &>(controller);
    ...
    // mc_rtc::log::info("[ResidualError] qdot: {}, tau_m: {}", qdot, tau_m);
    ...
    Eigen::VectorXd p_hat_dot = gamma + gamma_1*(p_error).cwiseAbs().cwiseSqrt().cwiseProduct(Sign(p_error)) + tau_ext_hat;
    p_hat += p_hat_dot*dt_;

    // d_hat_dot = γ2*Sign(x - x_hat)
    tau_ext_hat_dot = gamma_2*Sign(p_error);
...
double ResidualError::sign(double x)
{
    if (x>0) return 1;
...
Eigen::VectorXd ResidualError::Sign(Eigen::VectorXd x)
{
...
void ResidualError::addPlot(std::vector<std::string> & plots, double counter, mc_control::MCGlobalController & ctl)
{
...
void ResidualError::addGui(mc_control::MCGlobalController & ctl)
{
    auto & gui = *ctl.controller().gui();
    gui.addElement({"Plugins", "ResidualError"},
...
void ResidualError::addLog(mc_control::MCGlobalController & ctl)
{
    ctl.controller().logger().addLogEntry("ResidualError_p", [this]() { return p; });
    ctl.controller().logger().addLogEntry("ResidualError_p_hat", [this]() { return p_hat; });
    ctl.controller().logger().addLogEntry("ResidualError_p_error", [this]() { return p_error; });
    ctl.controller().logger().addLogEntry("ResidualError_tau_ext_hat", [this]() { return tau_ext_hat; });
    ctl.controller().logger().addLogEntry("ResidualError_tau_ext_hat_dot", [this]() { return tau_ext_hat_dot; });
    ctl.controller().logger().addLogEntry("ResidualError_gamma", [this]() { return gamma; });
}

void ResidualError::updateCounter(double counter)
{
    counter_ = counter;
}